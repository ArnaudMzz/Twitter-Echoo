<?php
    ...
    $stmt = $db->prepare("SELECT * FROM User WHERE id_user = :id_receiver");
    ...
    // Vérifier si les utilisateurs se suivent mutuellement
    $areMutualFollowers = User::areMutualFollowers($_SESSION['user']['id_user'], $id_receiver);

    // Récupérer les messages entre les deux utilisateurs
    $messages = User::getMessages($_SESSION['user']['id_user'], $id_receiver);

    // Récupérer les noms d'utilisateur des expéditeurs et des destinataires
    $usernames = [];
    foreach ($messages as $message) {
        if (!isset($usernames[$message['id_sender']])) {
            $stmt = $db->prepare("SELECT username FROM User WHERE id_user = :id_user");
            $stmt->bindParam(':id_user', $message['id_sender']);
            $stmt->execute();
            $usernames[$message['id_sender']] = $stmt->fetchColumn();
        }
        if (!isset($usernames[$message['id_receiver']])) {
            $stmt = $db->prepare("SELECT username FROM User WHERE id_user = :id_user");
            $stmt->bindParam(':id_user', $message['id_receiver']);
            $stmt->execute();
            $usernames[$message['id_receiver']] = $stmt->fetchColumn();
        }
    }
    ?>
    ...
    <head>
        ...
        <title>Messages avec <?= htmlspecialchars($receiver['username']) ?></title>
        ...
        <a
          ...
          <span class="ml-2 text-md font-bold">Compte</span>
            ...
            <main class="main-content">
                <header>
                    <h1 class="ml-2 text-xxl font-bold">Messages avec <?= htmlspecialchars($receiver['username']) ?></h1>
                </header>
                <div class="messages">
                    ...
                    <?php else : ?>
                        <?php foreach ($messages as $message) : ?>
                            <div class="message <?= $message['id_sender'] == $_SESSION['user']['id_user'] ? 'sent' : 'received' ?>">
                                <!-- Afficher le pseudo de l'expéditeur -->
                                <div class="message-header">
                                    <span class="username">
                                        <a href="Profile.php?username=<?= htmlspecialchars($usernames[$message['id_sender']]) ?>">
                                            <?= htmlspecialchars($usernames[$message['id_sender']]) ?>
                                        </a>
                                    </span>
                                </div>
                ...
                <?php if ($areMutualFollowers) : ?>
                    <form id="messageForm" method="POST">
                        <input type="hidden" name="id_receiver" value="<?= $id_receiver ?>">
                        <textarea name="content" id="messageContent" placeholder="Écrivez votre message..." required></textarea>
                        <button type="submit">Envoyer</button>
                    </form>
                <?php else : ?>
                    <p>Vous ne pouvez pas envoyer de message à cet utilisateur car vous ne vous suivez pas mutuellement.</p>
                <?php endif; ?>
            </main>
        </div>
    </body>
