...
<div class="contain-home">
    ...
    <main class="main-content">
      <header>
        <h1 class="text-3xl font-bold text-center border-b border-white pb-2">Accueil</h1>
      </header>
      <!-- Implémentation code bichon -->
      <div>
        <h1 class="top-username">Echoos tes idées ! : </h1>
        <div class="tweet-box">
        <form action="../Controller/TweetController.php" method="POST" enctype="multipart/form-data">
        <div class="tweet-container">
          <textarea name="content" placeholder="Quoi de neuf ?" class="tweet-area" required maxlength="140"></textarea>
          <div id="char-count" class="char-count">140</div>
        </div>
          <script>
            const tweetArea = document.querySelector('.tweet-area');
            const charCount = document.getElementById('char-count');
                tweetArea.addEventListener('input', () => {
                  const remainingChars = 140 - tweetArea.value.length;
                ...
                <button type="submit" class="tweet-btn">Echoo</button>
            </form>
        </div>

        <h2 class="lasted-tweets">Echoos récents</h2>
        <?php if (empty($tweets)) : ?>
            ...
            <div class="tweets">
              <div class="tweet-content">
                <p class="tweet-header">
                  <strong>
                    <a href="Profile.php?username=<?= urlencode($tweet['username']) ?>" class="tweet-username">
              ...
              <a class="tweet-btn response" href="#" data-tweet-id="<?= $tweet['id_tweet'] ?>">Répondre</a>
              <div class="responses" id="responses-<?= $tweet['id_tweet'] ?>"></div>
            </div>
          <?php endforeach; ?>
        <?php endif; ?>
        <!-- Exemple de tweet -->
        <div class="tweets">
          <div class="tweet-content">
            ...
            <p>
              Ceci est un exemple d'echo. Bienvenue sur cette page inspirée de
              Twitter !
            </p>
          </div>
        </div>
          ...
          <?php if (!empty($mentionedUsers)) : ?>
            <?php foreach ($mentionedUsers as $user) : ?>
              <div class="suggestion-item" onclick="window.location.href='Profile.php?username=<?= $user['username'] ?>'">
                <img src="<?= !empty($user['avatar_url']) ? $user['avatar_url'] : $default_avatar; ?>" alt="<?= htmlspecialchars($user['username']) ?>" class="user-avatar">
