...
$hashtag = $_GET['tag'] ?? null;

if (!$hashtag) {
    die("Erreur : Aucun hashtag spécifié.");
}
...
$stmt = $db->prepare("
    ...
    JOIN Hash_list ON Tweet.id_tweet = Hash_list.id_tweet 
    JOIN Hashtag ON Hash_list.id_hash = Hashtag.id_hash 
...
<head>
    ...
    <title>Hashtag #<?= htmlspecialchars($hashtag) ?></title>
    <link rel="stylesheet" href="./assets/Hashtag.css" />
    ...
    <div class="sidebar">
      ...
      <div
        ...
        <main class="main-content">
            <header>
                <h1 class="ml-2 text-xxl font-bold">Echoos avec #<?= htmlspecialchars($hashtag) ?></h1>
            </header>
            <div>
                <?php if (empty($tweets)) : ?>
                    <p>Aucun tweet trouvé pour ce hashtag.</p>
                <?php else : ?>
                    <?php foreach ($tweets as $tweet) : ?>
                        <div class="tweets">
                            <div class="tweet-content">
                                <strong class="tweet-header"><?= htmlspecialchars($tweet['username']) ?>:</strong>
